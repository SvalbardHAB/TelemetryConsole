...
#const_default_serial_port = "/dev/tty.usbserial-A6009CSG"
const_default_serial_port = "/dev/cu.usbmodem1657141"
...
def printData():
    ...
    if latest[3] in cloud_boards:
        print("Cloud density: {0}".format(latest[15]))
...
def processdata(received):
    ...
    try:
        latest[0]=int(split[0]) #CRC pass/fail
        latest[1] = int(split[1]) #RSSI
        latest[2] = int(split[2]) #SNR
        ...
        latest[13] = 0 #ex efm bytes—RIP EFM...
        latest[14] = 0
        if latest[3] in cloud_boards:
            latest[15] = (int(split[21],16)<<8 | int(split[22],16))  -512  #cloud detector average

            latest[16] = 0 #now overlooked.
        else:
            latest[15] = 0
            latest[16] = 0
        if latest[3] in hygro_boards:
            latest[17] = int(split[23],16) #relative humidity
            latest[18] = int(split[24],16) -128 #external temperature in excess 128 format
        else:
            latest[17] = 0
            latest[18] = 0
